...
export class LibraryManager {
    constructor(db, toast, modal) {
        ...
        byId('library-search').oninput = ev => {
            this.render(filterByQuery(this.songs, ev.target.value.toLowerCase()));
        };
        byId('refresh-library-btn').onclick = () => this.load();
        byId('select-all-checkbox').onchange = ev => {
            document.querySelectorAll('.song-check').forEach(cb => {
                cb.checked = ev.target.checked;
                if (ev.target.checked) this.selected.add(cb.value);
    ...
    render(songs) {
        ...
        tbody.querySelectorAll('.song-check').forEach(cb => {
            cb.onchange = () => {
    ...
    openAddToPlaylistModal(songIds) {
        ...
        byId('modal-confirm').onclick = async () => {
            ...
            await this.db.collection('playlists').update(chosenId, { songs: [...existingSet] });
