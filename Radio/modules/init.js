...
async function init() {
    ...
    // Snapshot of radio_state taken before a channel overrides the broadcast,
    // used to restore the default broadcast when the user stops listening.
    let savedDefaultState = null;
    ...
    channelMgr.onListenToChannel = async (channel) => {
        try {
            ...
            if (!channelMgr.activeChannelId) {
                savedDefaultState = await db.kv.get('radio_state').catch(() => null);
            }
            ...
            Object.assign(livePlayer.rules, channelRules);
    ...
    channelMgr.onStopChannel = async () => {
        try {
            ...
            if (savedDefaultState?.songs?.length) {
                await db.kv.set('radio_state', { ...savedDefaultState });
                savedDefaultState = null;
            } else {
                await db.kv.delete('radio_state').catch(() => {});
                savedDefaultState = null;
                livePlayer.setOffline();
            }
     ...
     */
    function activateView(viewName, skipHash = false) {
        ...
        if (viewName === 'channels') {
            ...
            channelMgr.load().then(() => {
                if (!skipHash && location.hash && location.hash !== '#') {
                    channelMgr.handleHash(location.hash);
                }
            });
        }
    }
    ...
    function handleHashNavigation(hash = location.hash) {
        if (hash?.startsWith('#/') && hash.length > 2) {
            activateView('channels', /* skipHash */ true);
            channelMgr.handleHash(hash);
        }
    }
    ...
    // Pre-load catalogue and channels when arriving via a deep-link hash
    if (location.hash?.startsWith('#/') && location.hash.length > 2) {
        loadCatSongs(db, catPlayer).then(async () => {
            await playlist.load();
