...
export class ChannelManager {
    ...
    async load() {
        ...
        if (!this.songs.length) {
            this.songs = await this.db.collection('songs').list().catch(() => []);
        }
        if (!this.playlists.length) {
            this.playlists = await this.db.collection('playlists').list().catch(() => []);
        }
    ...
    async openDetail(channelId, pushState = true) {
        // Ensure cache is populated before rendering detail
        if (!this.songs.length) {
            this.songs = await this.db.collection('songs').list().catch(() => []);
        }
        if (!this.playlists.length) {
