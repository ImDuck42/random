...
export class PlaylistManager {
    ...
    renderList() {
        ...
        container.innerHTML = this.playlists.map(pl => `
            <div class="playlist-item ${pl.id === this.activeId ? 'active' : ''}"
    ...
    populateDropdown() {
        ...
        select.innerHTML = `<option value="">Choose a playlist...</option>` +
            this.playlists.map(pl => `<option value="${pl.id}">${escapeHtml(pl.name)}</option>`).join('');
    }
    ...
    async openEditor(playlistId) {
        ...
        const tracks = (await Promise.all(
            (playlist.songs ?? []).map(sid => this.db.collection('songs').get(sid))
        )).filter(Boolean);
                ...
                ${tracks.length
                    ? tracks.map((song, idx) => `
                        <div class="pl-song-item" data-id="${song.id}">
                            ...
                            <span class="pl-song-num">${idx + 1}</span>
