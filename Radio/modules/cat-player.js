...
export class CatPlayer {
     ...
     * @param {LivePlayer|null} livePlayer - injected after construction
     */
    constructor(audioEl, router, volumeCtrl, toast, livePlayer) {
        ...
        byId('cat-progress-track').onclick = event => this.seek(event);
        byId('cat-search').oninput        = event => {
            this.renderGrid(filterByQuery(this.songs, event.target.value));
        };
    }
    ...
    /** Updates the sticky player bar with the current track's info. */
    updateBar() {
    ...
    /**
     * Renders the song grid. Accepts a filtered subset or falls back to all songs.
     * @param {object[]} displaySongs
     */
    ...
    /** Starts the requestAnimationFrame progress bar loop. */
    startProgressLoop() {
