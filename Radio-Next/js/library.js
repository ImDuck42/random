...
export class LibraryManager {
    constructor(db, toast, modal) {
        ...
        const searchEl = byId('library-search');
        if (searchEl) searchEl.oninput = event => {
            this.render(filterByQuery(this.songs, event.target.value.toLowerCase()));
        };
        const refreshBtn = byId('refresh-library-btn');
        if (refreshBtn) refreshBtn.onclick = () => this.load();
        const selectAll = byId('select-all-checkbox');
        if (selectAll) selectAll.onchange = event => {
        ...
        const bulkDel = byId('bulk-delete-btn');
        if (bulkDel) bulkDel.onclick = () => this.bulkDelete();
        const addToPl = byId('add-to-playlist-btn');
        if (addToPl) addToPl.onclick = () => {
    ...
    async load() {
        ...
        const totalEl = byId('lib-total');
        if (totalEl) totalEl.textContent = this.songs.length;
        const durEl = byId('lib-duration');
        if (durEl) durEl.textContent = formatDuration(totalMs);
        ...
        tbody.innerHTML = songs.map((song, index) => `
            <tr data-id="${song.id}">
                <td><input type="checkbox" class="song-check" value="${song.id}"
                           aria-label="Select ${escapeHtml(song.title ?? 'song')}" ${this.selected.has(song.id) ? 'checked' : ''}></td>
    ...
    openAddToPlaylistModal(songIds) {
        ...
        byId('modal-body').querySelectorAll('.pl-select-item').forEach(item => {
            item.onclick = () => {
                byId('modal-body').querySelectorAll('.pl-select-item').forEach(el => el.classList.remove('selected'));
