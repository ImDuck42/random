...
export class ChannelManager {
    constructor(db, toast, modal) {
        ...
        const searchEl = byId('ch-search');
        if (searchEl) searchEl.oninput = event => {
            this.renderGrid(filterByQuery(this.channels, event.target.value, ['name', 'description']));
        };
        const backBtn = byId('ch-back-btn');
        if (backBtn) backBtn.onclick = () => this.closeDetail();
    }
    ...
    async load() {
        ...
        if (!this.songs.length) this.songs = await this.db.collection('songs').list().catch(() => []);
        if (!this.playlists.length) this.playlists = await this.db.collection('playlists').list().catch(() => []);
        const countEl = byId('ch-count');
        if (countEl) countEl.textContent = `${this.channels.length} channel${this.channels.length !== 1 ? 's' : ''}`;
    ...
    async openDetail(channelId, pushState = true) {
        if (!this.songs.length) this.songs = await this.db.collection('songs').list().catch(() => []);
        if (!this.playlists.length) this.playlists = await this.db.collection('playlists').list().catch(() => []);
    ...
    async handleHash(hash) {
        ...
        await this.openDetail(channel.id, false);
        return channel;
    }
...
export class AdminChannelPanel {
    constructor(db, toast, modal, channelManager) {
        ...
        const newBtn = byId('new-channel-btn');
        if (newBtn) newBtn.onclick = () => this.openCreateModal();
    }
            ...
            <div class="rules-grid" style="margin-top:1.25rem;">
                ...
                <div class="admin-card">
                    ...
                    <div class="rule-row vertical">
                        ...
                        <input type="number" id="ch-form-sync" value="${channel.syncInterval ?? 10}" min="5" max="60" class="number-input">
                    ...
                    <div class="rule-row vertical">
                        ...
                        <input type="number" id="ch-form-drift" value="${channel.driftThreshold ?? 2}" min="1" max="10" class="number-input">
